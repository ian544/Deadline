...
class LoadTask(private val activity: AppCompatActivity) {
    ...
    fun execute() {
        Executors.newSingleThreadExecutor()
            .execute {
                ...
                val tasksjson = jsontasks.getCourses()
    ...
    private fun updateDisplay(courseList: ArrayList<Task>) {
        setupRecyclerView(courseList)
        Toast.makeText(appContext, "File loaded", Toast.LENGTH_LONG).show()
    }

    private fun setupRecyclerView(courseList: ArrayList<Task>) {
        recyclerView?.adapter = TaskAdapter(activity, courseList)
    }
